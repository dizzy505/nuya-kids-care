...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action'])) {
    ...
    elseif ($_POST['action'] == 'update') {
        $editId = $_POST['id'];
        $editNamaJabatan = $_POST['edit_nama_jabatan'];

        $stmt = $pdo->prepare("UPDATE jabatan SET nama_jabatan = ? WHERE id = ?");
        $stmt->execute([$editNamaJabatan, $editId]);
...
$stmtJabatan = $pdo->query("SELECT id, nama_jabatan FROM jabatan");
...
if ($editId) {
    ...
    if ($jabatan) {
        $editNamaJabatan = $jabatan['nama_jabatan'];
    }
}
?>
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-bold mb-4">Input Data Jabatan</h1>
        <?php if (isset($success)): ?>
            <div class="bg-green-100 text-green-700 p-2 mb-4 rounded"><?php echo $success; ?></div>
        <?php endif; ?>
        <form method="POST" action="jabatan.php" class="mb-4">
            <?php if ($editId): ?>
                <input type="hidden" name="action" value="update">
                <input type="hidden" name="id" value="<?php echo $editId; ?>">
                <div class="mb-4">
                    <label for="edit_nama_jabatan" class="block text-gray-700">Nama Jabatan</label>
                    <input type="text" name="edit_nama_jabatan" id="edit_nama_jabatan" class="border p-2 w-full" value="<?php echo $editNamaJabatan; ?>" required>
                </div>
                <button type="submit" class="mt-2 bg-yellow-500 text-white px-4 py-2 rounded">Update</button>
                <a href="jabatan.php" class="ml-2 text-gray-600">Batal</a>
            <?php else: ?>
                <input type="hidden" name="action" value="add">
                <div class="mb-4">
                    <label for="nama_jabatan" class="block text-gray-700">Nama Jabatan</label>
                    <input type="text" name="nama_jabatan" id="nama_jabatan" class="border p-2 w-full" required>
                </div>
        ...
        <div class="mb-4">
            <h2 class="text-xl font-bold mb-2">Daftar Jabatan</h2>
            <table class="min-w-full divide-y divide-gray-200 shadow-md rounded-lg overflow-hidden">
                <thead class="bg-gray-800">
                    <tr>
                        <th class="border px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Nama Jabatan</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($jabatanList as $jabatan): ?>
                        <tr>
                            <td class="border border-gray-200 px-4 py-2"><?php echo $jabatan['nama_jabatan']; ?></td>
                            <td class="border border-gray-200 px-4 py-2">
                                <!-- Tombol Edit -->
                                <a href="jabatan.php?edit_id=<?php echo $jabatan['id']; ?>" class="text-blue-500 hover:text-blue-700">Edit</a>

                                <!-- Tombol Delete -->
                                <form method="POST" action="jabatan.php" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data jabatan ini?');">
