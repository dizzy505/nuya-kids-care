...
$stmt = $pdo->query("SELECT id, nama FROM anak ORDER BY nama ASC");
...
$attendance_query = $pdo->prepare("
    SELECT anak_id, tanggal, jam_masuk, jam_pulang, durasi_belajar, lokasi_masuk, lokasi_pulang 
    FROM presensi_anak 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($type == 'masuk') {
        $check_masuk = $pdo->prepare("SELECT * FROM presensi_anak WHERE anak_id = :anak_id AND tanggal = :tanggal");
        $check_masuk->execute(['anak_id' => $anak_id, 'tanggal' => $tanggal]);
        
        if ($check_masuk->rowCount() == 0) {
            $stmt = $pdo->prepare("INSERT INTO presensi_anak (anak_id, tanggal, jam_masuk, lokasi_masuk, durasi_belajar) VALUES (:anak_id, :tanggal, :jam_masuk, :lokasi_masuk, 0)");
            $stmt->execute([
                'anak_id' => $anak_id, 
                'tanggal' => $tanggal, 
        ...
        $check_stmt = $pdo->prepare("SELECT * FROM presensi_anak WHERE anak_id = :anak_id AND tanggal = :tanggal AND jam_pulang IS NULL");
        $check_stmt->execute(['anak_id' => $anak_id, 'tanggal' => $tanggal]);
        ...
        if ($existing_record) {
            ...
            $durasi_belajar = $interval->h + ($interval->i / 60);

            $stmt = $pdo->prepare("UPDATE presensi_anak SET 
            ...
            $stmt->execute([
                ...
                'durasi_belajar' => number_format($durasi_belajar, 2, '.', '')
            ]);
...
function calculateTotalHours($records) {
    ...
    foreach ($records as $record) {
        if (!empty($record['durasi_belajar'])) {
            $total += floatval($record['durasi_belajar']);
        }
    }
    ...
    <div class="container mx-auto p-4">
        <section class="hero py-12">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md mb-6">
                ...
                <form method="POST" action="presensiAnak.php" id="presensiForm">
                    <div class="mb-4">
                        <label for="anak_id" class="block text-gray-700 mb-2">Nama Anak</label>
                        <select name="anak_id" id="anak_id" class="border rounded p-2 w-full" required>
